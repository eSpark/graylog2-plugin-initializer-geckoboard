...
public class StreamCountResponse implements ResponseBuilder {
    ...
    public String build(QueryStringDecoder qsd) throws StreamNotFoundException {
        ...
        if (params != null) {
            if (params.containsKey("stream_id") && !params.get("stream_id").get(0).isEmpty()) {
                streamId = params.get("stream_id").get(0);
            } else {
                throw new RuntimeException("Missing parameter: stream_id.");
            }
            
            if (params.containsKey("timeframe") && !params.get("timeframe").get(0).isEmpty()) {
                timeframe = Integer.parseInt(params.get("timeframe").get(0));
            } else {
                timeframe = STANDARD_TIMEFRAME;
            }
        } else {
